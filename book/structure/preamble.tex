...
\newcommand{\chaptercover}[2]{%
  \cleardoublepage
  \thispagestyle{empty} % 홀수 페이지
   \
  \newpage
  \thispagestyle{empty} % 짝수 페이지
  \drawlinesfalse % 선 긋기 중단
  \restoregeometry % 일반 용지로 변경
  \vspace*{\fill}
  \begin{center}{\large #1}\end{center} % 중앙 내용
  \vspace*{\fill}
  \begin{center}{\small #2}\end{center} % 하단 내용
  \newpage}
